package net.ramgaming.leapers.block;

import net.minecraft.block.Block;
import net.minecraft.util.function.BooleanBiFunction;
import net.minecraft.util.shape.VoxelShape;
import net.minecraft.util.shape.VoxelShapes;

import java.util.stream.Stream;

public class ModShapes {

    public static final VoxelShape REDSTONE_RAY_NORTH = Stream.of(
            Block.createCuboidShape(0, 0, 0, 16, 2, 16),
            Block.createCuboidShape(4, 4, 4, 12, 12, 12),
            Block.createCuboidShape(0, 3, 0, 2, 13, 2),
            Block.createCuboidShape(0, 2, 13, 3, 3, 16),
            Block.createCuboidShape(13, 2, 13, 16, 3, 16),
            Block.createCuboidShape(13, 2, 0, 16, 3, 3),
            Block.createCuboidShape(0, 2, 0, 3, 3, 3),
            Block.createCuboidShape(14, 3, 0, 16, 13, 2),
            Block.createCuboidShape(14, 3, 14, 16, 13, 16),
            Block.createCuboidShape(0, 3, 14, 2, 13, 16),
            Block.createCuboidShape(14, 2, 5, 16, 3, 11),
            Block.createCuboidShape(6, 5, 2, 10, 11, 3),
            Block.createCuboidShape(5.5, 7, 1, 6.5, 9, 2),
            Block.createCuboidShape(9.5, 7, 1, 10.5, 9, 2),
            Block.createCuboidShape(7, 5.5, 1, 9, 6.5, 2),
            Block.createCuboidShape(7, 9.5, 1, 9, 10.5, 2),
            Block.createCuboidShape(7, 7, 1.5, 9, 9, 2),
            Block.createCuboidShape(4, 2, 4, 12, 3, 12),
            Block.createCuboidShape(5, 6, 2, 6, 10, 3),
            Block.createCuboidShape(10, 6, 2, 11, 10, 3),
            Block.createCuboidShape(13, 13, 13, 16, 14, 16),
            Block.createCuboidShape(0, 13, 13, 3, 14, 16),
            Block.createCuboidShape(0, 14, 0, 16, 16, 16),
            Block.createCuboidShape(0, 13, 0, 3, 14, 3),
            Block.createCuboidShape(13, 13, 0, 16, 14, 3),
            Block.createCuboidShape(4, 13, 4, 12, 14, 12),
            Block.createCuboidShape(5, 2, 14, 11, 3, 16),
            Block.createCuboidShape(0, 2, 5, 2, 3, 11)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape REDSTONE_RAY_EAST = Stream.of(
            Block.createCuboidShape(0, 0, 0, 16, 2, 16),
            Block.createCuboidShape(4, 4, 4, 12, 12, 12),
            Block.createCuboidShape(14, 3, 0, 16, 13, 2),
            Block.createCuboidShape(0, 2, 0, 3, 3, 3),
            Block.createCuboidShape(0, 2, 13, 3, 3, 16),
            Block.createCuboidShape(13, 2, 13, 16, 3, 16),
            Block.createCuboidShape(13, 2, 0, 16, 3, 3),
            Block.createCuboidShape(14, 3, 14, 16, 13, 16),
            Block.createCuboidShape(0, 3, 14, 2, 13, 16),
            Block.createCuboidShape(0, 3, 0, 2, 13, 2),
            Block.createCuboidShape(5, 2, 14, 11, 3, 16),
            Block.createCuboidShape(13, 5, 6, 14, 11, 10),
            Block.createCuboidShape(14, 7, 5.5, 15, 9, 6.5),
            Block.createCuboidShape(14, 7, 9.5, 15, 9, 10.5),
            Block.createCuboidShape(14, 5.5, 7, 15, 6.5, 9),
            Block.createCuboidShape(14, 9.5, 7, 15, 10.5, 9),
            Block.createCuboidShape(14, 7, 7, 14.5, 9, 9),
            Block.createCuboidShape(4, 2, 4, 12, 3, 12),
            Block.createCuboidShape(13, 6, 5, 14, 10, 6),
            Block.createCuboidShape(13, 6, 10, 14, 10, 11),
            Block.createCuboidShape(0, 13, 13, 3, 14, 16),
            Block.createCuboidShape(0, 13, 0, 3, 14, 3),
            Block.createCuboidShape(0, 14, 0, 16, 16, 16),
            Block.createCuboidShape(13, 13, 0, 16, 14, 3),
            Block.createCuboidShape(13, 13, 13, 16, 14, 16),
            Block.createCuboidShape(4, 13, 4, 12, 14, 12),
            Block.createCuboidShape(0, 2, 5, 2, 3, 11),
            Block.createCuboidShape(5, 2, 0, 11, 3, 2)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape REDSTONE_RAY_SOUTH = Stream.of(
            Block.createCuboidShape(0, 0, 0, 16, 2, 16),
            Block.createCuboidShape(4, 4, 4, 12, 12, 12),
            Block.createCuboidShape(14, 3, 14, 16, 13, 16),
            Block.createCuboidShape(13, 2, 0, 16, 3, 3),
            Block.createCuboidShape(0, 2, 0, 3, 3, 3),
            Block.createCuboidShape(0, 2, 13, 3, 3, 16),
            Block.createCuboidShape(13, 2, 13, 16, 3, 16),
            Block.createCuboidShape(0, 3, 14, 2, 13, 16),
            Block.createCuboidShape(0, 3, 0, 2, 13, 2),
            Block.createCuboidShape(14, 3, 0, 16, 13, 2),
            Block.createCuboidShape(0, 2, 5, 2, 3, 11),
            Block.createCuboidShape(6, 5, 13, 10, 11, 14),
            Block.createCuboidShape(9.5, 7, 14, 10.5, 9, 15),
            Block.createCuboidShape(5.5, 7, 14, 6.5, 9, 15),
            Block.createCuboidShape(7, 5.5, 14, 9, 6.5, 15),
            Block.createCuboidShape(7, 9.5, 14, 9, 10.5, 15),
            Block.createCuboidShape(7, 7, 14, 9, 9, 14.5),
            Block.createCuboidShape(4, 2, 4, 12, 3, 12),
            Block.createCuboidShape(10, 6, 13, 11, 10, 14),
            Block.createCuboidShape(5, 6, 13, 6, 10, 14),
            Block.createCuboidShape(0, 13, 0, 3, 14, 3),
            Block.createCuboidShape(13, 13, 0, 16, 14, 3),
            Block.createCuboidShape(0, 14, 0, 16, 16, 16),
            Block.createCuboidShape(13, 13, 13, 16, 14, 16),
            Block.createCuboidShape(0, 13, 13, 3, 14, 16),
            Block.createCuboidShape(4, 13, 4, 12, 14, 12),
            Block.createCuboidShape(5, 2, 0, 11, 3, 2),
            Block.createCuboidShape(14, 2, 5, 16, 3, 11)
            ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape REDSTONE_RAY_WEST = Stream.of(
            Block.createCuboidShape(0, 0, 0, 16, 2, 16),
            Block.createCuboidShape(4, 4, 4, 12, 12, 12),
            Block.createCuboidShape(0, 3, 14, 2, 13, 16),
            Block.createCuboidShape(13, 2, 13, 16, 3, 16),
            Block.createCuboidShape(13, 2, 0, 16, 3, 3),
            Block.createCuboidShape(0, 2, 0, 3, 3, 3),
            Block.createCuboidShape(0, 2, 13, 3, 3, 16),
            Block.createCuboidShape(0, 3, 0, 2, 13, 2),
            Block.createCuboidShape(14, 3, 0, 16, 13, 2),
            Block.createCuboidShape(14, 3, 14, 16, 13, 16),
            Block.createCuboidShape(5, 2, 0, 11, 3, 2),
            Block.createCuboidShape(2, 5, 6, 3, 11, 10),
            Block.createCuboidShape(1, 7, 9.5, 2, 9, 10.5),
            Block.createCuboidShape(1, 7, 5.5, 2, 9, 6.5),
            Block.createCuboidShape(1, 5.5, 7, 2, 6.5, 9),
            Block.createCuboidShape(1, 9.5, 7, 2, 10.5, 9),
            Block.createCuboidShape(1.5, 7, 7, 2, 9, 9),
            Block.createCuboidShape(4, 2, 4, 12, 3, 12),
            Block.createCuboidShape(2, 6, 10, 3, 10, 11),
            Block.createCuboidShape(2, 6, 5, 3, 10, 6),
            Block.createCuboidShape(13, 13, 0, 16, 14, 3),
            Block.createCuboidShape(13, 13, 13, 16, 14, 16),
            Block.createCuboidShape(0, 14, 0, 16, 16, 16),
            Block.createCuboidShape(0, 13, 13, 3, 14, 16),
            Block.createCuboidShape(0, 13, 0, 3, 14, 3),
            Block.createCuboidShape(4, 13, 4, 12, 14, 12),
            Block.createCuboidShape(14, 2, 5, 16, 3, 11),
            Block.createCuboidShape(5, 2, 14, 11, 3, 16)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_NORTH = Stream.of(
            Block.createCuboidShape(4, 12, 12, 12, 14, 14),
            Block.createCuboidShape(-8, 0, 11, 8, 16, 12),
            Block.createCuboidShape(12, 0, 12, 14, 14, 14),
            Block.createCuboidShape(12, 12, 4, 14, 14, 12),
            Block.createCuboidShape(12, 0, 4, 14, 2, 12),
            Block.createCuboidShape(4, 0, 12, 12, 2, 14),
            Block.createCuboidShape(-8, 0, 12, 8, 16, 13)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_EAST = Stream.of(
            Block.createCuboidShape(2, 12, 4, 4, 14, 12),
            Block.createCuboidShape(8.349242404917499, 0, 2.4999999999999982, 9.349242404917499, 16, 18.5),
            Block.createCuboidShape(2, 0, 12, 4, 14, 14),
            Block.createCuboidShape(4, 12, 12, 12, 14, 14),
            Block.createCuboidShape(4, 0, 12, 12, 2, 14),
            Block.createCuboidShape(2, 0, 4, 4, 2, 12),
            Block.createCuboidShape(7.349242404917499, 0, 2.4999999999999982, 8.349242404917499, 16, 18.5)
            ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_SOUTH = Stream.of(
            Block.createCuboidShape(4, 12, 2, 12, 14, 4),
            Block.createCuboidShape(0.2071067811865479, 0, 7.813708498984759, 16.20710678118655, 16, 8.81370849898476),
            Block.createCuboidShape(2, 0, 2, 4, 14, 4),
            Block.createCuboidShape(2, 12, 4, 4, 14, 12),
            Block.createCuboidShape(2, 0, 4, 4, 2, 12),
            Block.createCuboidShape(4, 0, 2, 12, 2, 4),
            Block.createCuboidShape(0.2071067811865479, 0, 6.813708498984759, 16.20710678118655, 16, 7.813708498984759)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();;

    public static final VoxelShape MIRROR_WEST = Stream.of(
            Block.createCuboidShape(12, 12, 4, 14, 14, 12),
            Block.createCuboidShape(0.2071067811865479, 0, 7.813708498984759, 16.20710678118655, 16, 8.81370849898476),
            Block.createCuboidShape(12, 0, 2, 14, 14, 4),
            Block.createCuboidShape(4, 12, 2, 12, 14, 4),
            Block.createCuboidShape(4, 0, 2, 12, 2, 4),
            Block.createCuboidShape(12, 0, 4, 14, 2, 12),
            Block.createCuboidShape(0.2071067811865479, 0, 6.813708498984759, 16.20710678118655, 16, 7.813708498984759)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();;

    public static final VoxelShape MIRROR_UP_NORTH = Stream.of(
            Block.createCuboidShape(2, 0, 12, 14, 2, 14),
            Block.createCuboidShape(2, 2, 12, 4, 11.25, 14),
            Block.createCuboidShape(12, 2, 12, 14, 11.25, 14),
            Block.createCuboidShape(0, 6, 8, 16, 8, 10),
            Block.createCuboidShape(0, 7, 9, 16, 9, 11),
            Block.createCuboidShape(0, 10, 12, 16, 12, 14),
            Block.createCuboidShape(0, 9, 11, 16, 11, 13),
            Block.createCuboidShape(0, 8, 10, 16, 10, 12),
            Block.createCuboidShape(0, 3, 5, 16, 5, 7),
            Block.createCuboidShape(0, 4, 6, 16, 6, 8),
            Block.createCuboidShape(0, 5, 7, 16, 7, 9),
            Block.createCuboidShape(0, 2, 4, 16, 4, 6),
            Block.createCuboidShape(0, 1, 3, 16, 3, 5),
            Block.createCuboidShape(0, 0, 2, 16, 2, 4)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_UP_EAST = Stream.of(
            Block.createCuboidShape(2, 0, 2, 4, 2, 14),
            Block.createCuboidShape(2, 2, 2, 4, 11.25, 4),
            Block.createCuboidShape(2, 2, 12, 4, 11.25, 14),
            Block.createCuboidShape(6, 6, 0, 8, 8, 16),
            Block.createCuboidShape(5, 7, 0, 7, 9, 16),
            Block.createCuboidShape(2, 10, 0, 4, 12, 16),
            Block.createCuboidShape(3, 9, 0, 5, 11, 16),
            Block.createCuboidShape(4, 8, 0, 6, 10, 16),
            Block.createCuboidShape(9, 3, 0, 11, 5, 16),
            Block.createCuboidShape(8, 4, 0, 10, 6, 16),
            Block.createCuboidShape(7, 5, 0, 9, 7, 16),
            Block.createCuboidShape(10, 2, 0, 12, 4, 16),
            Block.createCuboidShape(11, 1, 0, 13, 3, 16),
            Block.createCuboidShape(12, 0, 0, 14, 2, 16)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_UP_SOUTH = Stream.of(
            Block.createCuboidShape(2, 0, 2, 14, 2, 4),
            Block.createCuboidShape(2, 2, 2, 4, 11.25, 4),
            Block.createCuboidShape(12, 2, 2, 14, 11.25, 4),
            Block.createCuboidShape(0, 0, 12, 16, 2, 14),
            Block.createCuboidShape(0, 1, 11, 16, 3, 13),
            Block.createCuboidShape(0, 2, 10, 16, 4, 12),
            Block.createCuboidShape(0, 5, 7, 16, 7, 9),
            Block.createCuboidShape(0, 4, 8, 16, 6, 10),
            Block.createCuboidShape(0, 3, 9, 16, 5, 11),
            Block.createCuboidShape(0, 8, 4, 16, 10, 6),
            Block.createCuboidShape(0, 9, 3, 16, 11, 5),
            Block.createCuboidShape(0, 10, 2, 16, 12, 4),
            Block.createCuboidShape(0, 7, 5, 16, 9, 7),
            Block.createCuboidShape(0, 6, 6, 16, 8, 8)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_UP_WEST = Stream.of(
            Block.createCuboidShape(12, 0, 2, 14, 2, 14),
            Block.createCuboidShape(12, 2, 12, 14, 11.25, 14),
            Block.createCuboidShape(12, 2, 2, 14, 11.25, 4),
            Block.createCuboidShape(8, 6, 0, 10, 8, 16),
            Block.createCuboidShape(9, 7, 0, 11, 9, 16),
            Block.createCuboidShape(12, 10, 0, 14, 12, 16),
            Block.createCuboidShape(11, 9, 0, 13, 11, 16),
            Block.createCuboidShape(10, 8, 0, 12, 10, 16),
            Block.createCuboidShape(5, 3, 0, 7, 5, 16),
            Block.createCuboidShape(6, 4, 0, 8, 6, 16),
            Block.createCuboidShape(7, 5, 0, 9, 7, 16),
            Block.createCuboidShape(4, 2, 0, 6, 4, 16),
            Block.createCuboidShape(3, 1, 0, 5, 3, 16),
            Block.createCuboidShape(2, 0, 0, 4, 2, 16)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_DOWN_NORTH = Stream.of(
            Block.createCuboidShape(2, 10, 12, 14, 12, 14),
            Block.createCuboidShape(12, 0, 12, 14, 10, 14),
            Block.createCuboidShape(2, 0, 12, 4, 10, 14),
            Block.createCuboidShape(12, 10, 1, 14, 12, 12),
            Block.createCuboidShape(2, 10, 1, 4, 12, 12),
            Block.createCuboidShape(12, 3, 8, 14, 5, 12),
            Block.createCuboidShape(2, 3, 8, 4, 5, 12),
            Block.createCuboidShape(0, 6, 4, 16, 8, 6),
            Block.createCuboidShape(0, 7, 3, 16, 9, 5),
            Block.createCuboidShape(0, 10, 0, 16, 12, 2),
            Block.createCuboidShape(0, 9, 1, 16, 11, 3),
            Block.createCuboidShape(0, 8, 2, 16, 10, 4),
            Block.createCuboidShape(0, 3, 7, 16, 5, 9),
            Block.createCuboidShape(0, 4, 6, 16, 6, 8),
            Block.createCuboidShape(0, 5, 5, 16, 7, 7),
            Block.createCuboidShape(0, 2, 8, 16, 4, 10),
            Block.createCuboidShape(0, 1, 9, 16, 3, 11),
            Block.createCuboidShape(0, 0, 10, 16, 2, 12)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_DOWN_EAST = Stream.of(
            Block.createCuboidShape(1, 10, 1, 3, 12, 13),
            Block.createCuboidShape(1, 0, 11, 3, 10, 13),
            Block.createCuboidShape(1, 0, 1, 3, 10, 3),
            Block.createCuboidShape(3, 10, 11, 14, 12, 13),
            Block.createCuboidShape(3, 10, 1, 14, 12, 3),
            Block.createCuboidShape(3, 3, 11, 7, 5, 13),
            Block.createCuboidShape(3, 3, 1, 7, 5, 3),
            Block.createCuboidShape(9, 6, -1, 11, 8, 15),
            Block.createCuboidShape(10, 7, -1, 12, 9, 15),
            Block.createCuboidShape(13, 10, -1, 15, 12, 15),
            Block.createCuboidShape(12, 9, -1, 14, 11, 15),
            Block.createCuboidShape(11, 8, -1, 13, 10, 15),
            Block.createCuboidShape(6, 3, -1, 8, 5, 15),
            Block.createCuboidShape(7, 4, -1, 9, 6, 15),
            Block.createCuboidShape(8, 5, -1, 10, 7, 15),
            Block.createCuboidShape(5, 2, -1, 7, 4, 15),
            Block.createCuboidShape(4, 1, -1, 6, 3, 15),
            Block.createCuboidShape(3, 0, -1, 5, 2, 15)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_DOWN_SOUTH = Stream.of(
            Block.createCuboidShape(2, 10, 0, 14, 12, 2),
            Block.createCuboidShape(2, 0, 0, 4, 10, 2),
            Block.createCuboidShape(12, 0, 0, 14, 10, 2),
            Block.createCuboidShape(2, 10, 2, 4, 12, 13),
            Block.createCuboidShape(12, 10, 2, 14, 12, 13),
            Block.createCuboidShape(2, 3, 2, 4, 5, 6),
            Block.createCuboidShape(12, 3, 2, 14, 5, 6),
            Block.createCuboidShape(0, 6, 8, 16, 8, 10),
            Block.createCuboidShape(0, 7, 9, 16, 9, 11),
            Block.createCuboidShape(0, 10, 12, 16, 12, 14),
            Block.createCuboidShape(0, 9, 11, 16, 11, 13),
            Block.createCuboidShape(0, 8, 10, 16, 10, 12),
            Block.createCuboidShape(0, 3, 5, 16, 5, 7),
            Block.createCuboidShape(0, 4, 6, 16, 6, 8),
            Block.createCuboidShape(0, 5, 7, 16, 7, 9),
            Block.createCuboidShape(0, 2, 4, 16, 4, 6),
            Block.createCuboidShape(0, 1, 3, 16, 3, 5),
            Block.createCuboidShape(0, 0, 2, 16, 2, 4)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();

    public static final VoxelShape MIRROR_DOWN_WEST = Stream.of(
            Block.createCuboidShape(13, 10, 2, 15, 12, 14),
            Block.createCuboidShape(13, 0, 2, 15, 10, 4),
            Block.createCuboidShape(13, 0, 12, 15, 10, 14),
            Block.createCuboidShape(2, 10, 2, 13, 12, 4),
            Block.createCuboidShape(2, 10, 12, 13, 12, 14),
            Block.createCuboidShape(9, 3, 2, 13, 5, 4),
            Block.createCuboidShape(9, 3, 12, 13, 5, 14),
            Block.createCuboidShape(5, 6, 0, 7, 8, 16),
            Block.createCuboidShape(4, 7, 0, 6, 9, 16),
            Block.createCuboidShape(1, 10, 0, 3, 12, 16),
            Block.createCuboidShape(2, 9, 0, 4, 11, 16),
            Block.createCuboidShape(3, 8, 0, 5, 10, 16),
            Block.createCuboidShape(8, 3, 0, 10, 5, 16),
            Block.createCuboidShape(7, 4, 0, 9, 6, 16),
            Block.createCuboidShape(6, 5, 0, 8, 7, 16),
            Block.createCuboidShape(9, 2, 0, 11, 4, 16),
            Block.createCuboidShape(10, 1, 0, 12, 3, 16),
            Block.createCuboidShape(11, 0, 0, 13, 2, 16)
    ).reduce((v1, v2) -> VoxelShapes.combineAndSimplify(v1, v2, BooleanBiFunction.OR)).get();
}
